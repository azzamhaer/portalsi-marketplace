<script lang="ts">
  import { onMount, onDestroy } from 'svelte';

  let { order, height = '320px' } = $props<{ order: any; height?: string }>();

  let mapEl: HTMLDivElement | null = $state(null);
  let map: any = null;
  let L: any = null;
  let marker: any = null;
  let progress = $state(0);
  let error = $state('');
  let timer: any;

  const vendor = $derived(order.items?.find((it: any) => it.vendor?.latitude && it.vendor?.longitude)?.vendor);
  const destination = $derived(order.address);
  const hasCoords = $derived(vendor?.latitude && vendor?.longitude && destination?.latitude && destination?.longitude);
  const etaDays = $derived(parseEtaDays(order.courier_eta));
  const transitHubs = [
    [-6.1256, 106.6559, 'CGK'],
    [-7.3798, 112.7870, 'SUB'],
    [-5.0616, 119.5540, 'UPG'],
    [-1.2683, 116.8945, 'BPN'],
    [3.6422, 98.8853, 'KNO'],
    [-8.7482, 115.1671, 'DPS'],
    [1.5493, 124.9259, 'MDC'],
    [-2.5769, 140.5164, 'DJJ'],
  ];

  function parseEtaDays(eta: string) {
    const nums = String(eta || '').match(/\d+/g)?.map(Number) ?? [];
    if (!nums.length) return 3;
    return Math.max(1, Math.max(...nums));
  }

  function loadScript(src: string) {
    return new Promise<void>((resolve, reject) => {
      if (document.querySelector(`script[src="${src}"]`)) return resolve();
      const s = document.createElement('script');
      s.src = src;
      s.onload = () => resolve();
      s.onerror = () => reject(new Error('Gagal memuat peta'));
      document.head.appendChild(s);
    });
  }

  function loadCss(href: string) {
    if (document.querySelector(`link[href="${href}"]`)) return;
    const l = document.createElement('link');
    l.rel = 'stylesheet';
    l.href = href;
    document.head.appendChild(l);
  }

  function routePoints() {
    const start = [Number(vendor.latitude), Number(vendor.longitude)];
    const end = [Number(destination.latitude), Number(destination.longitude)];
    if (distanceKm(start, end) > 350) {
      const originHub = nearestHub(start);
      const destinationHub = nearestHub(end);
      const points = [start, originHub, destinationHub, end];
      return dedupePoints(points);
    }
    return [start, [(start[0] + end[0]) / 2, (start[1] + end[1]) / 2], end];
  }

  function distanceKm(a: number[], b: number[]) {
    const toRad = (n: number) => n * Math.PI / 180;
    const dLat = toRad(b[0] - a[0]);
    const dLng = toRad(b[1] - a[1]);
    const lat1 = toRad(a[0]);
    const lat2 = toRad(b[0]);
    const h = Math.sin(dLat / 2) ** 2 + Math.cos(lat1) * Math.cos(lat2) * Math.sin(dLng / 2) ** 2;
    return 6371 * 2 * Math.atan2(Math.sqrt(h), Math.sqrt(1 - h));
  }

  function nearestHub(point: number[]) {
    const hub = transitHubs
      .map((h) => ({ h, d: distanceKm(point, [h[0] as number, h[1] as number]) }))
      .sort((a, b) => a.d - b.d)[0]?.h;
    return hub ? [hub[0] as number, hub[1] as number] : point;
  }

  function dedupePoints(points: number[][]) {
    return points.filter((p, i) => i === 0 || distanceKm(p, points[i - 1]) > 20);
  }

  function calcProgress() {
    if (['ARRIVED', 'DONE', 'REFUNDED'].includes(order.status)) return 1;
    const shippedAt = order.shipped_at ? new Date(order.shipped_at).getTime() : Date.now();
    const duration = etaDays * 24 * 60 * 60 * 1000;
    return Math.max(0.02, Math.min(0.98, (Date.now() - shippedAt) / duration));
  }

  function pointAt(points: number[][], t: number) {
    const seg = Math.min(points.length - 2, Math.floor(t * (points.length - 1)));
    const local = (t * (points.length - 1)) - seg;
    const a = points[seg], b = points[seg + 1];
    return [a[0] + (b[0] - a[0]) * local, a[1] + (b[1] - a[1]) * local];
  }

  function render() {
    if (!map || !hasCoords) return;
    progress = calcProgress();
    const pts = routePoints();
    const current = pointAt(pts, progress);
    marker?.setLatLng(current);
  }

  async function init() {
    if (!mapEl || !hasCoords) return;
    loadCss('https://unpkg.com/leaflet@1.9.4/dist/leaflet.css');
    await loadScript('https://unpkg.com/leaflet@1.9.4/dist/leaflet.js');
    L = (window as any).L;
    const pts = routePoints();
    map = L.map(mapEl, { zoomControl: true, attributionControl: true });
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19, attribution: '&copy; OpenStreetMap' }).addTo(map);
    L.polyline(pts, { color: '#2563eb', weight: 4, opacity: 0.8, dashArray: '8 8' }).addTo(map);
    L.marker(pts[0]).addTo(map).bindPopup('Alamat seller');
    L.marker(pts[pts.length - 1]).addTo(map).bindPopup('Alamat penerima');
    marker = L.marker(pointAt(pts, calcProgress()), {
      icon: L.divIcon({ className: '', html: '<div style="width:18px;height:18px;border-radius:999px;background:#16a34a;border:3px solid white;box-shadow:0 4px 12px rgba(0,0,0,.25)"></div>' })
    }).addTo(map);
    map.fitBounds(L.latLngBounds(pts), { padding: [28, 28] });
    timer = setInterval(render, 60_000);
    render();
  }

  onMount(() => init().catch(() => error = 'Peta pengiriman belum bisa dimuat.'));
  onDestroy(() => { clearInterval(timer); map?.remove(); });
</script>

{#if hasCoords}
  <div class="overflow-hidden rounded-2xl border border-ink-200 bg-ink-50">
    <div bind:this={mapEl} style:height class="w-full"></div>
    <div class="flex items-center justify-between gap-3 px-3 py-2 text-xs text-ink-600">
      <span>Progress estimasi: {Math.round(progress * 100)}%</span>
      <span>ETA {order.courier_eta || `${etaDays} hari`}</span>
    </div>
  </div>
{:else}
  <div class="rounded-2xl border border-ink-100 bg-ink-50 p-4 text-sm text-ink-600">
    Peta rute belum tersedia karena koordinat seller atau penerima belum lengkap.
  </div>
{/if}
{#if error}
  <div class="mt-2 rounded-xl bg-amber-50 px-3 py-2 text-xs text-amber-800">{error}</div>
{/if}
